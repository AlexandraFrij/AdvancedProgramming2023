...
public class Main {
    ...
    /**
     * the method creates a matrix in which each line is a permutation of numbers from 1 to n (the dimension of the matrix)
        ...
        for (int i = 0; i < dimension; i++)
            for (int j = 0; j < dimension; j++)
                if (j == 0)
                    ...
                    stringLines[i] = stringLines[i] + matrix[i][j];
        return stringLines;
    }
